/* 
 * Ported changes in from https://github.com/mattrobenolt/nginx-dogstatsd
 * to support tagging of metrics
 * 
 */
...
ngx_http_statsd_handler(ngx_http_request_t *r)
{
	...
	for (c = 0; c < ulcf->stats->nelts; c++) {
		...
		t = ngx_http_statsd_key_get_value(r, stat.ctags, stat.tags);
...
ngx_http_statsd_add_stat(ngx_conf_t *cf, ngx_command_t *cmd, void *conf, ngx_uint_t type) {
	...
	if (cf->args->nelts > 3) {
		...
		if (tags_cv.lengths == NULL) {
			s = ngx_http_statsd_key_value(&value[3]);
			/*if (n < 0) {
		...
		if (cf->args->nelts > 4) {
			...
			if (valid_cv.lengths == NULL) {
				b = ngx_http_statsd_valid_value(&value[4]);
				if (b < 0) {
